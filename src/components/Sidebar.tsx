...
import { Blend, Bot, Cat, Clover, Compass, Container, Film, Globe, Home, Menu, Music, Play, Search, Star, Tv, TvMinimalPlay, Users } from 'lucide-react';
...
const Sidebar = ({ onToggle, activePath = '/' }: SidebarProps) => {
  ...
  const [menuItems, setMenuItems] = useState([
    ...
    },
    {
      ...
      action: 'openDirectPlay' as const,
    },
    {
      icon: Bot,
      label: 'AI问片',
                      ...
                      <button
                        ...
                        onClick={() => {
                          window.dispatchEvent(
                            new CustomEvent((item as any).action)
                          );
                        }}
                        className={`group flex items-center rounded-lg px-2 py-2 pl-4 text-sm text-gray-700 hover:bg-gray-100/30 hover:text-green-600 transition-colors duration-200 min-h-[40px] dark:text-gray-300 dark:hover:text-green-400 ${isCollapsed ? 'w-full max-w-none mx-0' : 'mx-0'
                          } gap-3 justify-start`}
                      >
                        <div className='w-4 h-4 flex items-center justify-center'>
                          <Icon className='h-4 w-4 text-gray-500 group-hover:text-green-600 dark:text-gray-400 dark:group-hover:text-green-400' />
                        </div>
                        {!isCollapsed && (
                          <span className='whitespace-nowrap transition-opacity duration-200 opacity-100'>
                            {item.label}
